...
def assemble_video(script, footage, voiceover, overlays, work_dir, output):
    ...
    if footage:
        ...
        fl.write_text("\n".join(f"file '{p.resolve()}'" for _ in range(math.ceil(total_dur/5)+2) for p in footage))
        subprocess.run(["ffmpeg","-y","-f","concat","-safe","0","-i",str(fl),"-t",str(total_dur),
            "-vf",f"scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H},setfps={FPS}",
            "-c:v","libx264","-preset","fast","-crf","23","-an",str(bg)], capture_output=True)
    else:
        subprocess.run(["ffmpeg","-y","-f","lavfi","-i",f"color=c=0x080C14:size={W}x{H}:rate={FPS}:duration={total_dur}",
            "-c:v","libx264",str(bg)], capture_output=True)
    ol_list = work_dir/"ol_list.txt"
    ol_list.write_text("\n".join(f"file '{p.resolve()}'" for p in overlays))
    ol_concat = work_dir/"overlays.mp4"
    subprocess.run(["ffmpeg","-y","-f","concat","-safe","0","-i",str(ol_list),
        "-c:v","libx264","-preset","fast","-crf","18","-pix_fmt","yuva420p",str(ol_concat)], capture_output=True)
