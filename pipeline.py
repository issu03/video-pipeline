...
def generate_voiceover(scenes, work_dir):
    ...
    if r_audio.returncode != 0:
        log.error(f"   ❌ audio concat failed: {r_audio.stderr[-300:]}")
        # Fallback: just use first audio file
        if audio_files and audio_files[0].exists():
            import shutil as _sh
            _sh.copy(str(audio_files[0]), str(final))
            log.info("   ⚠️ Using first audio file as fallback")
    if final.exists():
        log.info(f"   ✅ Voiceover done ({final.stat().st_size/1024:.0f}KB)")
    else:
        log.error("   ❌ Voiceover file missing!")
    return final
...
def create_overlays(scenes, work_dir):
    ...
    for si, scene in enumerate(scenes):
        ...
        for f in range(n_frames):
            ...
            img_rgb.save(frames_dir/f"frame_{f:04d}.png")
        ...
        if r_ov.returncode != 0:
            log.error(f"   ❌ overlay {si} failed: {r_ov.stderr[-200:]}")
        else:
            log.info(f"   ✅ overlay {si}: {ov.stat().st_size/1024:.0f}KB")
